...
class ActionHeadFinetune(nn.Module):
    def __init__(self, dropout_ratio=0., dim_feat=512, num_classes=60, num_joints=18, hidden_dim=2048):
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x[:, :, 2:4, :], x[:, :, 5:8, :], x[:, :, 9:12, :], x[:, :, 13:16, :], x[:, :, 17:20, :],
                       x[:, :, 21:25, :]), dim=-2)
        ...
        x = self.embedding.expmap_with_projection(x)  # 投影至双曲空间
